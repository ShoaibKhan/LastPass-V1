...
def display_passowrd(event):
    ...
    if not (website and user_name and master_password and alphabet and length):
        return

    length = int(length)
    password = generate_password(
        website, user_name, master_password, length, alphabet)
...
lower_state, upper_state, nums_state, special_char_state = IntVar(
    value=1), IntVar(value=1), IntVar(value=1), IntVar(value=1)

lower = Checkbutton(root, text="lowercase", fg="#00FF00",bg="Black", variable=lower_state)
upper = Checkbutton(root, text="UPPERCASE", fg="#00FF00",
                    bg="Black", variable=upper_state)
nums = Checkbutton(root, text="Numbers", fg="#00FF00",
                   bg="Black", variable=nums_state)
special_char = Checkbutton(
    root, text="Special Characters", fg="#00FF00", bg="Black", variable=special_char_state)
...
button_generate = Button(root, text="Generate", height=3,
                         width=15, bg="Red", fg="Black")
